...
X = filtered_data[['date']].values.astype(int)  # Use date as a feature (you might need to preprocess it further)
y = filtered_data['price'].values 
# the below line needs modification, tipange split ma prices into seperate colums to match either maizeprice or beans price
# y = filtered_data[option + 'price'].values
 # Adjust column name to match your actual data
...
forecast_dates = pd.date_range(start=end_date + datetime.timedelta(days=1), periods=7)  # Change the number of periods as needed
forecast_features = filtered_data[['date']].tail(1).values.astype(int)  # Use the last available date

forecast_prices = []
for _ in range():  # Change the range as needed
...
ax.plot(filtered_data['date'], filtered_data['price'], label='Historical Prices', marker='o')

# the below line needs modification, tipange split ma prices into seperate colums to match either maizeprice or beans price

# ax.plot(filtered_data['date'], filtered_data[option +'price'], label='Historical Prices', marker='o')
...
# version yomwe tinasiila last week

# from here is the version before adding prophet
import streamlit as st
from pathlib import Path
import pandas as pd
import datetime
